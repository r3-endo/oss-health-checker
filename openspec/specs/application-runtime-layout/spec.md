...
The system MUST separate runnable applications under `apps/` as `apps/frontend`, `apps/backend`, and `apps/batch`.
...
The system MUST provide documented local run commands for backend and frontend during and after layout migration.

#### Scenario: backend をローカル起動できる
- **WHEN** 開発者が README の手順に従って backend 起動コマンドを実行する
- **THEN** backend はローカルで起動できなければならない

#### Scenario: frontend をローカル起動できる
- **WHEN** 開発者が README の手順に従って frontend 起動コマンドを実行する
- **THEN** frontend はローカルで起動できなければならない
